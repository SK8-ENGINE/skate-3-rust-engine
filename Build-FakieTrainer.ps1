...
$privateDirectory = Join-Path $PSScriptRoot '.local/fakie-trainer'
$buildDirectory = Join-Path $privateDirectory 'build'
$artifact = Join-Path $buildDirectory 'skate3rust-fakie-trainer.exe'
$destination = Join-Path $privateDirectory 'skate3-fakie-trainer.exe'
...
try {
    ...
    if (-not $StageOnly) {
        ...
        & cargo rustc --release --locked --target x86_64-pc-windows-msvc `
            ...
            --bin skate3rust -- -C extra-filename=-fakie-trainer `
    ...
    foreach ($required in @('--start-paused', 'Scroll wheel: more settings', 'Drag // to resize', 'hold_fakie', 'Native trainer controls are already owned by another mod')) {
        if (-not $text.Contains($required)) { throw "Wrong fakie-trainer artifact: missing $required" }
    }
