...
$privateDirectory = Join-Path $PSScriptRoot '.local/lua-sdk'
$buildDirectory = Join-Path $privateDirectory 'build'
$artifact = Join-Path $buildDirectory 'skate3rust-lua-sdk.exe'
$destination = Join-Path $privateDirectory 'skate3-lua-sdk.exe'
...
try {
    ...
    if (-not $StageOnly) {
        ...
        & cargo rustc --release --locked --target x86_64-pc-windows-msvc `
            ...
            --bin skate3rust -- -C extra-filename=-lua-sdk `
    ...
    foreach ($required in @('--start-paused', 'MODS', 'Lua instruction budget exhausted')) {
        if (-not $text.Contains($required)) { throw "Wrong lua-sdk artifact: missing $required" }
    }
