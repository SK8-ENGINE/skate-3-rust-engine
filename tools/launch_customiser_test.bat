...
setlocal
cd /d "%~dp0"
if not exist "%~dp0skate3rust-customiser-v3.exe" (
  echo The V3 customiser executable is missing.
  pause
  exit /b 1
)
...
echo Skate 3 Character Customiser V3
...
echo Diagnostic output: customiser-v3-session.log
"%~dp0skate3rust-customiser-v3.exe" --assets "%~dp0assets" --map "%~dp0maps\SkateSchool.skate" > "%~dp0customiser-v3-session.log" 2>&1
set "skate_customiser_exit=%errorlevel%"
if not "%skate_customiser_exit%"=="0" (
  echo The test exited with code %skate_customiser_exit%.
  echo See %~dp0customiser-v3-session.log
  pause
)
