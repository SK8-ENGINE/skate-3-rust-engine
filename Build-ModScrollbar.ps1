...
$privateDirectory = Join-Path $PSScriptRoot '.local/mod-scrollbar'
$buildDirectory = Join-Path $privateDirectory 'build'
$artifact = Join-Path $buildDirectory 'skate3rust-mod-scrollbar.exe'
$destination = Join-Path $privateDirectory 'skate3-mod-scrollbar.exe'
...
try {
    ...
    if (-not $StageOnly) {
        ...
        & cargo rustc --release --locked --target x86_64-pc-windows-msvc `
            ...
            --bin skate3rust -- -C extra-filename=-mod-scrollbar `
    ...
    foreach ($required in @('--start-paused', 'Wheel or drag scrollbar', 'Drag // to resize', 'hold_fakie', 'Native trainer controls are already owned by another mod')) {
        if (-not $text.Contains($required)) { throw "Wrong mod-scrollbar artifact: missing $required" }
    }
