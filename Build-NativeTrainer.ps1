...
$privateDirectory = Join-Path $PSScriptRoot '.local/native-trainer'
$buildDirectory = Join-Path $privateDirectory 'build'
$artifact = Join-Path $buildDirectory 'skate3rust-native-trainer.exe'
$destination = Join-Path $privateDirectory 'skate3-native-trainer.exe'
...
try {
    ...
    if (-not $StageOnly) {
        ...
        & cargo rustc --release --locked --target x86_64-pc-windows-msvc `
            ...
            --bin skate3rust -- -C extra-filename=-native-trainer `
    ...
    foreach ($required in @('--start-paused', 'ENABLED MOD SETTINGS', 'Native trainer controls are already owned by another mod')) {
        if (-not $text.Contains($required)) { throw "Wrong native-trainer artifact: missing $required" }
    }
