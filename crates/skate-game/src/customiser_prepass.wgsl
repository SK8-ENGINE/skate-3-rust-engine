// Bevy 0.18.1 pbr_prepass.wgsl, MIT/Apache-2.0, copyright Bevy contributors.
// Match native hair coverage in colour, depth and shadow passes.
struct SkinStamp {
    transform: vec4<f32>,
    rectangle: vec4<f32>,
    enabled: vec4<f32>,
...
fn customiser_alpha_discard(in: prepass_io::VertexOutput) {
#ifdef VERTEX_UVS_B
    let alpha = textureSample(hair_opacity, hair_sampler, in.uv_b).r;
    if stamp.enabled.y > 0.5 {
        let coverage = pbr_functions::alpha_discard(pbr_bindings::material, vec4<f32>(1.0, 1.0, 1.0, alpha));
        return;
    }
#endif
