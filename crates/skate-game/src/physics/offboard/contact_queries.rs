...
use skate_core::{
    math::Vector3,
    physics::{board_world::{BoardWorld, query_metadata::QueryPool},
        drive_frames::RetailAffineTransform, triangle_query::{TriangleLineHit, triangle_segment}},
...
pub(crate) fn submit(world: &BoardWorld, layout: &Layout, input: Input, matching_id: u32)
    -> Result<Completed, String>
{
    ...
    Ok(Completed { input, support: [a?,b?,c?], obstacles: hits })
}

fn xyz(v: [f32;4]) -> Vector3 { Vector3::new(v[0],v[1],v[2]) }
fn query(world: &BoardWorld, probe: Probe, matching_id: u32) -> Result<Option<Hit>, String> {
    ...
    let candidates: Vec<_> = if metadata.meshes.iter().all(|m| m.local_to_world == RetailAffineTransform::IDENTITY) {
        world.line_candidates(start, end, probe.radius).map(|(i,_)| i).collect()
    } else { (0..world.triangles().len()).collect() };
    for pool in [QueryPool::Ground, QueryPool::Island, QueryPool::Conditional] {
        ...
        for (mesh_index, mesh) in metadata.meshes.iter().enumerate() {
            if mesh.pool != pool || !(mesh.matching_group == -1 || mesh.matching_group == matching_id as i32) { continue; }
            ...
            for &index in &candidates[first..last] {
                ...
                let mut geometry = TriangleLineHit { position: Vector3::ZERO, normal: Vector3::ZERO,
                    fraction: 0., volume_parameter: [0.;3] };
                if triangle_segment(&mut geometry,start,delta,vertices,probe.radius,triangle.fatness)
                    && nearest.as_ref().is_none_or(|h| geometry.fraction < h.geometry.fraction)
                {
                    nearest = Some(Hit { geometry, packed_surface: metadata.packed_surfaces[index],
                        mesh_index, support_frame: frame });
                }
            }
        }
    }
...
mod tests {
    ...
    fn authored_floor_and_empty_space_produce_real_support_observations() {
        let world = crate::physics::ground::world(skate_core::physics::contact::RetailContactMaterial {
            static_friction: 0., dynamic_friction: 0., restitution: 0.,
        });
        let mut input = Input { position: [0.,0.,0.,0.], surface_right: [1.,0.,0.,0.],
