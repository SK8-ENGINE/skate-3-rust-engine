...
fn stick_controls_pitch_and_roll_only_in_air_without_adding_lift() {
    for (pitch,steering) in [(1.,0.),(-1.,0.),(0.,1.),(0.,-1.)] {
        ...
        s.vehicles.get_mut(&id).unwrap().controls=Controls{pitch,steering,..Default::default()};
        for _ in 0..60 {s.step(1./120.);assert!(s.take_ejection(id).is_none());}
        let body=&s.world.bodies[s.vehicles[&id].body];
        let angular=body.rotation().inverse()*body.angvel();
        if pitch!=0. {assert!(angular.x*pitch>1.);}
        if steering!=0. {assert!(angular.z*steering< -1.);}
...
fn low_speed_wall_bump_does_not_eject_driver() {
    let (mut s,id)=setup(0.6);
    for _ in 0..240 {s.step(1./120.);}
    s.world.insert(RigidBodyBuilder::fixed().translation(Vector::new(0.,1.,3.)),ColliderBuilder::cuboid(10.,2.,0.1));
    s.world.step();
    let h=s.vehicles[&id].body;
    s.world.bodies[h].set_linvel(Vector::Z*5.,true);
    for _ in 0..180 {s.step(1./120.);assert!(s.take_ejection(id).is_none());}
...
fn sustained_grounded_inversion_still_ejects() {
    ...
    for _ in 0..480 {s.step(1./120.);if let Some(e)=s.take_ejection(id) {ejection=Some(e);break;}}
    assert_eq!(ejection.expect("grounded inverted wreck must eject").reason,"inverted");
}
...
fn ground_stabilizer_reduces_roll_disturbance() {
    ...
    assert!(rates[1]<rates[0]-0.02,"roll rates without/with assist: {rates:?}");
}
