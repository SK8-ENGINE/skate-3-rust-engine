...
use bevy_render::{
    ...
    render_resource::{
        BindGroup, BindGroupLayoutId, BindingResource, Buffer, BufferSize, Sampler, TextureView,
...
pub struct LightmapSlab {
    ...
    // A slab's textures are static between insert/remove calls. The model
    // buffer contents may change without changing its binding identity.
    cached_bind_groups: Vec<CachedLightmapBindGroup>,
}

struct CachedLightmapBindGroup {
    model: Buffer,
    offset: u64,
...
impl LightmapSlab {
    ...
    pub(crate) fn cached_bind_group(
        ...
        let BindingResource::Buffer(model) = model else {
            return None;
        };
        self.cached_bind_groups
            .iter()
            .find(|cached| {
                &*cached.model == model.buffer
                    && cached.offset == model.offset
                    && cached.size == model.size
                    && cached.layout == layout
            })
            .map(|cached| cached.bind_group.clone())
    }

    pub(crate) fn cache_bind_group(
        &mut self,
        model: &BindingResource,
        layout: BindGroupLayoutId,
        bind_group: BindGroup,
        ...
        // Bound retained buffers even when phases disappear or buffers resize.
        // Eviction only rebuilds a binding; it cannot change rendering.
        if self.cached_bind_groups.len() == 8 {
            self.cached_bind_groups.remove(0);
        }
        self.cached_bind_groups.push(CachedLightmapBindGroup {
            model: Buffer::from(model.buffer.clone()),
            offset: model.offset,
            size: model.size,
            layout,
            bind_group,
