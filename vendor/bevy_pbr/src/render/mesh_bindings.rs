...
impl MeshLayouts {
    ...
    pub fn lightmapped(
        ...
        let layout = pipeline_cache.get_bind_group_layout(&self.lightmapped);
        if let Some(cached) = lightmap_slab.cached_bind_group(model, layout.id()) {
            return cached;
        }
        let bind_group = if bindless_lightmaps {
        ...
        lightmap_slab.cache_bind_group(model, layout.id(), bind_group.clone());
        bind_group
    }
