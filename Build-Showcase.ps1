...
$privateDirectory = Join-Path $PSScriptRoot '.local/showcase'
$buildDirectory = Join-Path $privateDirectory 'build'
$artifact = Join-Path $buildDirectory 'skate3rust-showcase.exe'
$destination = Join-Path $privateDirectory 'skate3-showcase.exe'
...
try {
    ...
    if (-not $StageOnly) {
        ...
        & cargo rustc --release --locked --target x86_64-pc-windows-msvc `
            ...
            --bin skate3rust -- -C extra-filename=-showcase `
    ...
    foreach ($required in @('--start-paused', 'Drag title to move', 'Native trainer controls are already owned by another mod')) {
        if (-not $text.Contains($required)) { throw "Wrong showcase artifact: missing $required" }
    }
